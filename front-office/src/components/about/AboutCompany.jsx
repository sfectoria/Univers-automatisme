...
import { fetchSection } from '@/store/sections'
import { useParams } from 'next/navigation'
function AboutCompany() {
  const { id} = useParams();

  const sectionC = useSelector((state) => state.sections.section)
  console.log(sectionC,"sectionC")

  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(fetchSection(9))
  }, [dispatch,id])
  useEffect(() => {
    AOS.init({
      offset: 100,
    ...
    <>
      ...
      <section className='pb-[3rem] pt-[3rem] md:pt-[8rem]' data-aos="fade-left">
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 w-[80%] mx-auto lg:gap-[7rem] gap-16 items-center'>
         ...
         {/* {sectionC && Array.isArray(sectionC.content[0]) && sectionC.content[0].map((item, index) =>{ */}
